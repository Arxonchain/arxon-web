...
const Corner = ({ pos }: { pos: "tl"|"tr"|"bl"|"br" }) => {
  ...
  return <div className={`absolute ${cls} w-3 h-3 ${b} border-[#a8c3f0]/25`} />;
};
  ...
  return (
    <section id="features" className="relative bg-[#09090b] py-24 md:py-32" ref={ref}>
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        {/* Desktop 3-col grid */}
        <div className="hidden md:grid grid-cols-3 gap-px bg-white/[0.04] rounded-xl overflow-hidden border border-white/[0.10]">
          {features.map((f,i) => (
            <motion.div key={i} initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{delay:i*0.05}}
              ...
              <div className="flex items-start justify-between mb-5">
                <div className="w-9 h-9 rounded-lg bg-[#a8c3f0]/8 border border-[#a8c3f0]/12 flex items-center justify-center group-hover:bg-[#a8c3f0]/14 transition-colors">
                  <f.icon size={16} className="text-[#a8c3f0]/60 group-hover:text-[#a8c3f0] transition-colors" />
                </div>
                <span className="font-mono text-[8px] text-white/55 tracking-widest">{f.id}</span>
              </div>
              <h4 className="text-white text-[14px] font-bold mb-2">{f.title}</h4>
              <p className="text-white/55 text-[12px] leading-relaxed mb-4">{f.desc}</p>
              <span className="font-mono text-[8px] text-[#a8c3f0]/30 bg-[#a8c3f0]/5 border border-[#a8c3f0]/10 px-2 py-0.5 rounded tracking-wider">{f.tag}</span>
            </motion.div>
          ))}
        </div>
        ...
        <div className="md:hidden">
          <div ref={scrollRef} onScroll={handleScroll}
            ...
            {features.map((f,i) => (
              <div key={i} className="snap-start shrink-0 w-[calc(50vw-24px)] relative bg-[#0a0a0d] border border-white/[0.10] rounded-xl p-5">
                <div className="w-8 h-8 rounded-lg bg-[#a8c3f0]/8 border border-[#a8c3f0]/12 flex items-center justify-center mb-4">
                  <f.icon size={14} className="text-[#a8c3f0]/60" />
                </div>
          ...
          <div className="flex items-center gap-3 mt-4 justify-center">
            <button onClick={()=>goTo(-1)} className="w-8 h-8 rounded-lg border border-white/[0.10] flex items-center justify-center text-white/60 hover:text-white/80 hover:border-[#a8c3f0]/25 transition-all"><ChevronLeft size={14}/></button>
            <div className="flex gap-1.5">
              {Array.from({length:totalPages}).map((_,i)=>(<div key={i} className={`h-1 rounded-full transition-all ${i===activeIndex?"w-6 bg-[#a8c3f0]":"w-1.5 bg-white/15"}`}/>))}
            </div>
            <button onClick={()=>goTo(1)} className="w-8 h-8 rounded-lg border border-white/[0.10] flex items-center justify-center text-white/60 hover:text-white/80 hover:border-[#a8c3f0]/25 transition-all"><ChevronRight size={14}/></button>
          </div>
        </div>
      </div>
    </section>
