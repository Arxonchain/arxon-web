...
const AmbassadorApply = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.full_name || !form.email || !form.x_handle || !form.arxon_account_id || !form.country || !form.motivation) {
          ...
          <div className="pt-28 pb-20 px-6">
            <div className="max-w-[600px] mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <motion.button onClick={() => navigate("/ambassadors/portal")} whileHover={{ scale: 1.03 }}
                  className="bg-[#7c93c3] text-white font-bold px-6 py-3 rounded-xl text-sm">
                  Go to My Portal
                </motion.button>
                <motion.button onClick={() => navigate("/ambassadors")} whileHover={{ scale: 1.03 }}
        ...
        <div className="pt-28 pb-20 px-6">
          <div className="max-w-[700px] mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
              ...
              <p className="text-[#a1a1aa] text-sm">Fill out the form below to start your ambassador journey</p>
            </motion.div>

            <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }} className="space-y-5 bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 md:p-8">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <input className={inputClass} type="email" placeholder="you@example.com" value={form.email} onChange={e => updateField("email", e.target.value)} />
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="text-white/70 text-xs font-medium mb-1.5 block">Twitter (X) Account Link *</label>
                  <input className={inputClass} placeholder="https://x.com/yourhandle" value={form.x_handle} onChange={e => updateField("x_handle", e.target.value)} />
                </div>
              </div>

              <div>
                <label className="text-white/70 text-xs font-medium mb-1.5 block">Country *</label>
                <input className={inputClass} placeholder="Your country" value={form.country} onChange={e => updateField("country", e.target.value)} />
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[1, 2, 3, 4, 5].map(n => (
                    <input key={n} className={inputClass}
              ...
              <div>
                ...
                <textarea className={`${inputClass} min-h-[100px]`} placeholder="Tell us your motivation..."
                  value={form.motivation} onChange={e => updateField("motivation", e.target.value)} />
              </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <input type="checkbox" required className="mt-0.5 accent-[#7c93c3] w-4 h-4 flex-shrink-0" />
                <span className="text-[#71717a] text-sm group-hover:text-[#a1a1aa] transition-colors">I have an active Arxon account</span>
              </label>

              <motion.button type="submit" disabled={loading}
                whileHover={{ scale: 1.02, boxShadow: "0 0 40px rgba(124,147,195,0.35)" }} whileTap={{ scale: 0.98 }}
                ...
                <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent"
                  animate={{ x: ["-200%", "200%"] }} transition={{ duration: 2, repeat: Infinity, ease: "linear", repeatDelay: 1 }} />
