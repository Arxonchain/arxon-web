...
const inputCls = "w-full bg-white/[0.03] border border-white/[0.07] rounded-lg px-4 py-3 text-white/85 text-sm font-mono placeholder:text-white/60 focus:outline-none focus:border-[#a8c3f0]/35 focus:bg-[#a8c3f0]/[0.02] transition-all pr-10";
      ...
      <motion.div initial={{opacity:0,y:24,scale:0.97}} animate={{opacity:1,y:0,scale:1}} className="relative max-w-[420px] w-full">
        <div className="relative bg-[#0a0a0d] border border-white/[0.10] rounded-2xl overflow-hidden">
          ...
          <div className="p-8">
            ...
            <div className="font-mono text-[9px] text-[#a8c3f0]/50 tracking-widest mb-2">{isLogin?"SIGN_IN":"REGISTER"}</div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={()=>setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/65 hover:text-white/50 transition-colors">
            ...
            <button onClick={()=>setIsLogin(!isLogin)} className="w-full text-center font-mono text-[10px] text-white/65 hover:text-white/50 transition-colors mt-4">
              {isLogin?"Don't have an account? Register →":"Have an account? Sign in →"}
            </button>
          </div>
        </div>
        <button onClick={()=>navigate("/")} className="flex items-center gap-2 font-mono text-[10px] text-white/60 hover:text-white/60 transition-colors mx-auto mt-4 w-fit">
